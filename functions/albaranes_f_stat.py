...
def obtener_fact_por_mes_client_last_3_years(nombre_cliente): # http://localhost:5000/api/alb_stat?cli_fact_3=grk
    ...
    for year in range(current_year - 2, current_year + 1):
        pipeline = [
            ...
            {"$match": {"FechaAlbaran": {"$gte": datetime(year, 1, 1), "$lt": datetime(year + 1, 1, 1)}}},
            {
                "$group": {
...
def obtener_ing_por_mes_client_cy(nombre_cliente):  # http://localhost:5000/api/alb_stat?cli_ing_cy=grk
    current_year = datetime.now().year  
    collection = db['CabeceraAlbaran']
    search_terms = nombre_cliente.split(',')
    search_string = " ".join(['"' + term + '"' for term in search_terms])
...
def obtener_ing_por_mes_client_last_3_years(nombre_cliente): # http://localhost:5000/api/alb_stat?cli_ing_3=grk
    current_year = datetime.now().year
    collection = db['CabeceraAlbaran']
    search_terms = nombre_cliente.split(',')
    search_string = " ".join(['"' + term + '"' for term in search_terms])
